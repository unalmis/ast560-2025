...
// First-order upwind
static inline double calc_flux_upwind_1o(double vel, double fll, double fl,
                                         double fr, double frr) {
  return 0.5 * vel * (fr + fl) - 0.5 * fabs(vel) * (fr - fl);
}

// Second-order central
static inline double calc_flux_central_2o(double vel, double fll, double fl,
                                          double fr, double frr) {
...
// Second-order central
static inline double calc_diff2_central_2o(double dx, double fll, double fl,
                                           double f0, double fr, double frr) {
  return (fl - 2.0 * f0 + fr) / (dx * dx);
}

// Fourth-order central
static inline double calc_diff2_central_4o(double dx, double fll, double fl,
                                           double f0, double fr, double frr) {
  return (-(fll + frr) + 16 * (fl + fr) - 30 * f0) / (12 * dx * dx);
}

// function pointer types for computing advective interface flux
typedef double (*advect_flux_t)(double vel, double fll, double fl, double fr,
                                double frr);

// function pointer types for computing diffusive term
typedef double (*diff_stencil_t)(double dx, double fll, double fl, double f0,
                                 double fr, double frr);
...
adiff_app *adiff_app_new(const struct adiff_app_inp *inp) {
  ...
  struct gkyl_rect_decomp *decomp =
      gkyl_rect_decomp_new_from_cuts(ndim, cuts, &app->local);
  app->comm =
      gkyl_null_comm_inew(&(struct gkyl_null_comm_inp){.decomp = decomp});
  gkyl_rect_decomp_release(decomp);

  return app;
}

static void apply_bc(adiff_app *app, struct gkyl_array *arr) {
  int per_dirs[] = {0, 1, 2};
  gkyl_comm_array_per_sync(app->comm, &app->local, &app->local_ext, app->ndim,
                           per_dirs, arr);
}

static void apply_ic(adiff_app *app) {
  struct gkyl_offset_descr offsets[] = {
      {0.0, 0.0, 0.0} // f is at cell-center
  };

  struct gkyl_eval_offset_fd_inp inp = {.grid = &app->grid,
                                        .num_ret_vals = 1,
                                        .offsets = offsets,
                                        .eval = app->init_func,
                                        .ctx = app->init_ctx};
...
static void calc_diagnostics(adiff_app *app, double tm) {
...
static void calc_velocity_field(adiff_app *app, double tm) {
  struct gkyl_offset_descr offsets[] = {
...
static void write_field(adiff_app *app, double tm, int frame,
...
static void write_f(adiff_app *app, double tm, int frame) {
  char fileNm[1024];
...
static void write_vel(adiff_app *app, double tm, int frame) {
...
static void write_data(struct gkyl_tm_trigger *iot, adiff_app *app,
                       double t_curr) {
...
static void write_diagnostics(adiff_app *app) {
  char fileNm[1024];
...
static inline long get_offset(int dir, int loc,
...
static double calc_max_dt(adiff_app *app) {
  double dx[3];
  ...
  gkyl_array_reduce_range(global_cfl_freq, app->cfl_freq, GKYL_MAX,
                          &app->face_range);
...
// stable time-step
static void advection_rhs(adiff_app *app, const struct gkyl_array *fin,
...
static void diffusion_rhs(adiff_app *app, const struct gkyl_array *fin,
                          struct gkyl_array *rhs) {
  int ndim = app->ndim;
  ...
  for (int d = 0; d < ndim; ++d) {
    ...
    while (gkyl_range_iter_next(&iter)) {
      ...
      rhs_p[0] +=
          alpha * diff_stencil(dx[d], fll[0], fl[0], f0[0], fr[0], frr[0]);
    }
  }
}

static void forward_euler(adiff_app *app, double tcurr, double dt,
...
static struct update_status update_ssp_rk3(adiff_app *app, double tcurr,
                                           double dt0) {
  ...
  gkyl_array_accumulate_range(
      gkyl_array_scale_range(app->f2, 1.0 / 4.0, &app->local), 3.0 / 4.0,
      app->f, &app->local);
  apply_bc(app, app->f2);
  ...
  gkyl_array_accumulate_range(
      gkyl_array_scale_range(app->fnew, 2.0 / 3.0, &app->local), 1.0 / 3.0,
      app->f, &app->local);
